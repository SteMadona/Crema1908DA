...
prep_physical_context <- function(df_raw,
  ...
  physical_data <- df_raw %>%
    transmute(
      ...
      SpeedZ1_m = `Speed Zone - Distance Covered (m) Zone 1 [0.0-1.5(m/s)]`,
      SpeedZ2_m = `Speed Zone - Distance Covered (m) Zone 2 [1.5-2.0(m/s)]`,
      SpeedZ3_m = `Speed Zone - Distance Covered (m) Zone 3 [2.0-3.0(m/s)]`,
      SpeedZ4_m = `Speed Zone - Distance Covered (m) Zone 4 [3.0-4.0(m/s)]`,
      SpeedZ5_m = `Speed Zone - Distance Covered (m) Zone 5 [4.0-5.5(m/s)]`,
      SpeedZ6_m = `Speed Zone - Distance Covered (m) Zone 6 [5.5-7.0(m/s)]`,
      SpeedZ7_m = `Speed Zone - Distance Covered (m) Zone 7 [> 7.0(m/s)]`,
      AccZ1_m   = `Horizontal Acc Zones - Distance Covered (m) Zone 1 [0.0-0.4(m/s^2)]`,
      ...
      AccZ4_m   = `Horizontal Acc Zones - Distance Covered (m) Zone 4 [1.5-2.0(m/s^2)]`,
      AccZ5_m   = `Horizontal Acc Zones - Distance Covered (m) Zone 5 [2.0-2.6(m/s^2)]`,
      AccZ6_m   = `Horizontal Acc Zones - Distance Covered (m) Zone 6 [> 2.6(m/s^2)]`,
      DecZ1_m   = `Horizontal Decl Zones - Distance Covered (m) Zone 1 [0.0-0.6(m/s^2)]`,
      ...
      DecZ5_m   = `Horizontal Decl Zones - Distance Covered (m) Zone 5 [2.2-2.8(m/s^2)]`,
      DecZ6_m   = `Horizontal Decl Zones - Distance Covered (m) Zone 6 [> 2.8(m/s^2)]`
    ) %>%
    filter(Tag == tag_keep) %>%
...
report_fisico <- function(giocatore, ctx, theme_fn = theme_crema_pro) {
  ...
  dati_plot <- bind_rows(
    dati_giocatore %>% mutate(tipo = .env$giocatore),
    media_ruolo    %>% mutate(tipo = "Media Ruolo")
  ) %>%
    drop_na(statistica) %>%
    mutate(
      statistica = factor(statistica, levels = ordine_variabili),
      tipo = factor(tipo, levels = c(.env$giocatore, "Media Ruolo"))
    )
