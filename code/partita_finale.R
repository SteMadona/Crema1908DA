...
partite <- c("CREMA vs PRO PALAZZOLO 1a", "CREMA vs PISTOIESE", "CREMA vs SANTANGELO 1a",
             "CREMA vs LENTIGIONE")


df_match <- df %>%
  filter(Tag %in% partite)
...
names(df_match) <- c("date", "player", "category", "position",
                  "touch_L", "touch_R", "touch_per_min",
                  "rv_z1", "rv_z2", "rv_z3", "rv_z4", "rv_z5", "rv_z6",
                  "pos_long", "pos_short", "one_touch",
                  "one_touch_R", "one_touch_L",
...
num_cols <- setdiff(names(df_match), c("date", "player","category","position"))
df_match[num_cols] <- lapply(df_match[num_cols], function(x) suppressWarnings(as.numeric(x)))
...
# normalizzazioni per confronto interno (min–max sui non-portieri, così l'indice è leggibile 0–100)
non_gk_match <- agg_match %>% filter(category != "Goalkeepers")

rng_rescale <- function(x, data = non_gk_match) rescale(x, to = c(0,100), from = range(data[[deparse(substitute(x))]], na.rm = TRUE))

# variabili di base per l’indice possesso
non_gk_match <- non_gk_match %>%
  mutate(
    receives_norm = rescale(receives, to = c(0,1), from = range(receives, na.rm = TRUE)),
    pos_long_share = share_long
  )
...
agg_match <- agg_match %>%
  left_join(non_gk_match %>% select(player, receives_norm, pos_long_share), by="player", relationship="many-to-many") %>%
  mutate(
    # indice possesso (composito, poi 0–100)
    possession_index_raw = W_POSSESSION["touch_per_min"]    * scales::rescale(touch_per_min, to=c(0,1), from=range(non_gk_match$touch_per_min, na.rm = TRUE)) +
      W_POSSESSION["releases_per_min"] * scales::rescale(releases_per_min, to=c(0,1), from=range(non_gk_match$releases_per_min, na.rm = TRUE)) +
      W_POSSESSION["pos_long_share"]   * pos_long_share +
      W_POSSESSION["receives_norm"]    * receives_norm,
    
    possession_index = rescale(possession_index_raw, to=c(0,100),
                               from=range(possession_index_raw[agg_match$category!="Goalkeepers"], na.rm = TRUE)),
    
    touches_index    = rescale(touches_total, to=c(0,100),
                               from=range(agg_match$touches_total[agg_match$category!="Goalkeepers"], na.rm = TRUE)),
    
    rv_index = rescale(rv_index_raw, to=c(0,100),
                       from=range(agg_match$rv_index_raw[agg_match$category!="Goalkeepers"], na.rm = TRUE)),
    
    time_ball_index = rescale(avg_time_per_poss_sec, to=c(0,100),
                              from=range(agg_match$avg_time_per_poss_sec[agg_match$category!="Goalkeepers"], na.rm = TRUE))
  )

# media ruolo (esclusi GK)
role_avg_match <- agg_match %>%
  filter(category != "Goalkeepers") %>%
  group_by(category) %>%
  summarise(
    possession_index = mean(possession_index, na.rm=TRUE),
...
# dataset “report” per giocatore selezionato + media ruolo
build_report_df_match <- function(player_name){
  ruolo <- agg_match %>% filter(player == player_name) %>% pull(category) %>% unique()
  media_ruolo_row <- role_avg_match %>% filter(category == ruolo)
  player_row <- agg_match %>% filter(player == player_name)
...
report_tecnico_match <- function(giocatore){
  ruolo <- agg_match %>% filter(player == giocatore) %>% pull(category) %>% unique()
  dati_plot <- build_report_df_match(giocatore)
