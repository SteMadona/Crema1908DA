...
df_physicalreport_week_u17 <- ctx$cph_u17$physical_data %>%
  ...
  left_join(ctx$cph_u17$physical_data %>% select(player, category) %>% distinct(), by = "player") %>%
  group_by(player) %>%
  mutate(
    acc_index = {
...
data_carichi_u17 <- df_physicalreport_week_u17 %>%
  group_by(week_id) %>%
  summarise(
    workrate   = mean(workrate, na.rm = TRUE),
...
tb_carichi_lavoro_u17 <- gt(data_carichi_u17) %>%
  tab_header(
