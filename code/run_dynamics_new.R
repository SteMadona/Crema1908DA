...
prep_run_context <- function(df_raw, 
                             players){
  df_dynamic <- df_raw %>%
  ...
  df_dynamic_match <- df_dynamic %>% 
    filter(tag %in% c("Partita finale martedi", "Partita finale mercoledi", 
                      "Match giovedi", "Partita finale venerdi", "Partita finale"))  
  ...
  list(
    df_dynamic_match = df_dynamic_match, 
    df_dynamic_report = df_dynamic_report, 
    df_dynamic_hid_match = df_dynamic_hid_match, 
    df_dynamic_hid_report = df_dynamic_hid_report
  )
