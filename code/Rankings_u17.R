...
data_topspeed <- ctx$cph_u17$physical_data %>%
...
data_workrate <- ctx$cph_u17$physical_data %>%
...
data_touches <- ctx$ctx_session_u17$df_skill %>%
  group_by(player) %>%
...
data_releases <- ctx$ctx_session_u17$df_skill %>%
  group_by(player) %>%
