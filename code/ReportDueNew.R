...
prep_skill_context <- function(df_raw,
  ...
  df_skill <- df_raw %>%
    transmute(
      ...
      rv_z1 = sum2_na(`MT - RV (#) Zone 1 [0.0-10.0( m/s)]`,
                      `OT - RV (#) Zone 1 [0.0-10.0( m/s)]`),
      
      rv_z2 = sum2_na(`MT - RV (#) Zone 2 [10.0-15.0( m/s)]`,
                      `OT - RV (#) Zone 2 [10.0-15.0( m/s)]`),
      
      rv_z3 = sum2_na(`MT - RV (#) Zone 3 [> 15.0( m/s)]`,
                      `OT - RV (#) Zone 3 [> 15.0( m/s)]`),
  ...
  add_derivations <- function(tbl) {
    tbl %>%
      mutate(
        ...
        rv_total = rv_z1 + rv_z2 + rv_z3,
        rv_index_raw =
          (RV_WEIGHTS[1]*rv_z1 + RV_WEIGHTS[2]*rv_z2 + RV_WEIGHTS[3]*rv_z3)  / pmax(rv_total, 1),
